...
def predict(model, data_loader, label_list):
    ...
    # 写入 CSV 文件
    with open('p2_政府车圈新闻_bert_wwmlarge_prediction.csv', 'w', newline='', encoding='utf-8') as file:
...
'''
CUDA_VISIBLE_DEVICES=0 python predict_p2.py    
'''
if __name__ == '__main__':
    ...
    '''
    # 加载模型检查点
    # checkpoint_model_path = os.path.join(os.getcwd(), 'save_bert_models', 'curr_ckpt', 'checkpoint_epoch_20.pt')
    ...
    large model
    '''
    checkpoint_model_path = os.path.join(os.getcwd(), 'save_bert_wwmlarge_models_p2_政府车圈新闻', 'curr_ckpt', 'checkpoint_epoch_6.pt')
    checkpoint_model, optimizer = load_ckp(checkpoint_model_path, model, optimizer)

    label_list = ['交通政策','补贴政策','汽车油价','二手车限迁法规','价格行情','花边新闻','销量新闻','新闻聚合','人物观点','行业评论','汽车出口','新能源新闻','论坛峰会']
