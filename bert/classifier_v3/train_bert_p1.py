...
def evaluate(model, data_loader):
    ...
    with open('train_p1_bert.log', 'a', encoding='utf-8') as log_file:
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(validation_loader, 0):
                ...
                if (batch_idx + 1) % eval_every_n_batches == 0:
                    ...
                    with open('train_bert.log', 'a',encoding='utf-8') as log_file:
                        for i, (p, r, f) in enumerate(zip(precision, recall, f1)):
                            label_name = target_list[i]
                            log_info = f'Label {label_name} - Precision: {p:.4f}, Recall: {r:.4f}, F1-Score: {f:.4f}\n'
                            print(log_info)
...
def predict(model, data_loader, label_list):
    ...
    # 写入 CSV 文件
    with open('p1_bert_wwm_large_predict.csv', 'w', newline='', encoding='utf-8') as file:
...
CUDA_VISIBLE_DEVICES=3 python train_bert_p1.py    
'''
if __name__ == '__main__':
    '''
    设置模型保存地址
    '''
    ckpt_path = os.path.join(os.getcwd(),'save_bert_wwmlarge_models_p1','curr_ckpt')
    best_model_path = os.path.join(os.getcwd(),'save_bert_wwmlarge_models_p1','best_model.pt')
    ...
    产生输出用测试集
    '''
    checkpoint_model_path = os.path.join(os.getcwd(), 'save_bert_wwmlarge_models_p1', 'curr_ckpt', 'checkpoint_epoch_8.pt')
    ...
    predict(model, test_data_loader, p1_label_list)
