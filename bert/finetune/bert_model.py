...
'''
class YicheBERT_Class(torch.nn.Module):
    def __init__(self):
        super(YicheBERT_Class, self).__init__()
        self.bert_model = BertModel.from_pretrained(Config.model_path, return_dict=True)
        self.linear = torch.nn.Linear(Config.in_features, Config.out_features)  #out_featues就是最终输出的类别数量
