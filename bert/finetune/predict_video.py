...
def predict(model, data_loader, label_list):
    ...
    # 写入 CSV 文件
    with open('P0_bert_wwmlarge_video_prediction.csv', 'w', newline='', encoding='utf-8') as file:
...
'''
CUDA_VISIBLE_DEVICES=3 python predict_video.py    
'''
if __name__ == '__main__':
    ...
    large model
    '''
    checkpoint_model_path = os.path.join(os.getcwd(), 'save_video_bert_wwmlarge_models_p0', 'curr_ckpt', 'checkpoint_epoch_8.pt')
    checkpoint_model, optimizer = load_ckp(checkpoint_model_path, model, optimizer)
    label_list = ['谍照', '实车曝光', '配置曝光', '申报图', '预热', '新车上市', '预售', '发布亮相', '新车官图',
                  '新车报价', '新车到店', '新车解析']
    predict(model, test_data_loader, label_list)
