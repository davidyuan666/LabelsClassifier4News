...
'''
all_data_csv_path = 'data/20230101_news_and_yichehao_news_p2_内容负向.csv'
...
def evaluate(model, data_loader):
    ...
    with open('train_p2_内容负向_bert.log', 'a', encoding='utf-8') as log_file:
...
def predict(model, data_loader, label_list):
    ...
    # 写入 CSV 文件
    with open('p2_内容负向_bert_wwm_large_predict.csv', 'w', newline='', encoding='utf-8') as file:
...
CUDA_VISIBLE_DEVICES=1 python train_bert_p2_内容负向.py    
'''
if __name__ == '__main__':
    '''
    设置模型保存地址
    '''
    ckpt_path = os.path.join(os.getcwd(),'save_bert_wwmlarge_models_p2_内容负向','curr_ckpt')
    best_model_path = os.path.join(os.getcwd(),'save_bert_wwmlarge_models_p2_内容负向','best_model.pt')
    ...
    产生输出用测试集
    '''
    checkpoint_model_path = os.path.join(os.getcwd(), 'save_bert_wwmlarge_models_p2_内容负向', 'curr_ckpt', 'best_model.pt')
    checkpoint_model, optimizer = load_ckp(checkpoint_model_path, model, optimizer)

    p2_label_list = ['交通事故', '自燃','维权事件','车辆减配','故障投诉','车辆召回','产能不足','车辆首撞','商家吐槽','爱车吐槽']
