...
'''
all_data_csv_path = 'data/20230101_news_and_yichehao_news_p2_政府车圈新闻.csv'
...
def evaluate(model, data_loader):
    ...
    with open('train_p2_政府车圈新闻_bert.log', 'a', encoding='utf-8') as log_file:
...
def predict(model, data_loader, label_list):
    ...
    # 写入 CSV 文件
    with open('p2_政府车圈新闻_bert_wwm_large_predict.csv', 'w', newline='', encoding='utf-8') as file:
...
CUDA_VISIBLE_DEVICES=0 python train_bert_p2_政府车圈新闻.py    
'''
if __name__ == '__main__':
    '''
    设置模型保存地址
    '''
    ckpt_path = os.path.join(os.getcwd(),'save_bert_wwmlarge_models_p2_政府车圈新闻','curr_ckpt')
    best_model_path = os.path.join(os.getcwd(),'save_bert_wwmlarge_models_p2_政府车圈新闻','best_model.pt')
    ...
    产生输出用测试集
    '''
    checkpoint_model_path = os.path.join(os.getcwd(), 'save_bert_wwmlarge_models_p2_政府车圈新闻', 'curr_ckpt', 'best_model.pt')
    checkpoint_model, optimizer = load_ckp(checkpoint_model_path, model, optimizer)

    p2_label_list = ['交通政策','补贴政策','汽车油价','二手车限迁法规','价格行情','花边新闻','销量新闻','新闻聚合','人物观点','行业评论','汽车出口','新能源新闻','论坛峰会']

    predict(model, test_data_loader, p2_label_list)
