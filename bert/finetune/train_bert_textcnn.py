...
'''
all_data_csv_path = 'data/all_multilabel_data.csv'
...
'''
target_list = ['谍照','实车曝光','配置曝光','申报图','预热','新车上市','预售','发布亮相','新车官图','新车报价','新车到店','新车解析','其他']
...
def evaluate(model, data_loader):
    ...
    for i, (p, r, f) in enumerate(zip(precision, recall, f1)):
        ...
        print(f'Label {label_name} - Precision: {p:.4f}, Recall: {r:.4f}, F1-Score: {f:.4f}')
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(validation_loader, 0):
                ...
                if (batch_idx + 1) % eval_every_n_batches == 0:
                    ...
                    for i, (p, r, f) in enumerate(zip(precision, recall, f1)):
                        print(f'Label {i+1} - Precision: {p:.4f}, Recall: {r:.4f}, F1-Score: {f:.4f}')
                    val_targets = []
                    val_outputs = []
...
CUDA_VISIBLE_DEVICES=2 python train_bert_textcnn.py    
'''
if __name__ == '__main__':
    '''
    设置模型保存地址
    '''
    ckpt_path = os.path.join(os.getcwd(),'save_bert_textcnn_models','curr_ckpt')
    best_model_path = os.path.join(os.getcwd(),'save_bert_textcnn_models','best_model.pt')
