...
from dataset_loader import YicheNewsDataset,YicheVideosDataset
...
'''
all_data_csv_path = 'data/20231101_yiche_video_p0.csv'
...
def evaluate(model, data_loader):
    ...
    with open('train_video_p0_wwm_bert.log', 'a', encoding='utf-8') as log_file:
...
def predict(model, data_loader, label_list):
    ...
    # 写入 CSV 文件
    with open('p0_video_bert_wwm_predict.csv', 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['VideoId','Content', 'True Labels', 'Predicted Labels'])
...
记得安装 transformers==4.36.2
CUDA_VISIBLE_DEVICES=3 python train_video_bert_p0.py    
'''
if __name__ == '__main__':
    '''
    设置模型保存地址
    '''
    ckpt_path = os.path.join(os.getcwd(),'save_video_wwmlarge_models_for_p0','curr_ckpt')
    best_model_path = os.path.join(os.getcwd(),'save_video_wwmlarge_models_for_p0','best_model.pt')
    ...
    产生输出用测试集
    '''
    checkpoint_model_path = os.path.join(os.getcwd(), 'save_video_wwmlarge_models_for_p0', 'curr_ckpt', 'checkpoint_epoch_8.pt')
