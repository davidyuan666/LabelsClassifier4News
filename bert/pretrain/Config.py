...
class Config(object):
    ...
    def __init__(self):
        ...
        self.sen_max_length = 512                                   # 句子最长长度
        self.padding = True                                         # 是否对输入进行padding

        # 模型及路径配置
        self.initial_pretrain_model = './pretrained_models/bert-base-chinese'           # 加载的预训练分词器checkpoint是bert-base-uncased，默认为英文。若要选择中文，替换成 bert-base-chinese
        self.initial_pretrain_tokenizer = './pretrained_models/bert-base-chinese'       # 加载的预训练模型checkpoint是bert-base-uncased，默认为英文。若要选择中文，替换成 bert-base-chinese
        self.path_model_save = './checkpoint/yiche_bert/'                      # 模型保存路径
