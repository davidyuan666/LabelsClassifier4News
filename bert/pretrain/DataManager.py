...
class DataManager(object):
    ...
    def get_dataset(self, mode='train', sampler=True):
        ...
        if mode=='train':
            train_dataloader = self.data_process('train_eng.txt', tokenizer)
            return train_dataloader
        elif mode=='dev':
            eval_dataloader = self.data_process('dev_eng.txt', tokenizer)
            return eval_dataloader
        else:
            test_dataloader = self.data_process('test_eng.txt', tokenizer, sampler=sampler)
    ...
    def data_process(self, file_name, tokenizer, sampler=True):
        ...
        # 转换成DataLoader类
        # train_dataloader = DataLoader(tokenized_datasets, shuffle=True, batch_size=configs.batch_size, collate_fn=data_collator)
        # eval_dataloader = DataLoader(tokenized_datasets_test, batch_size=configs.batch_size, collate_fn=data_collator)
        # sampler = RandomSampler(tokenized_datasets) if not torch.cuda.device_count() > 1 else DistributedSampler(tokenized_datasets)
        # dataloader = DataLoader(tokenized_datasets, sampler=sampler, batch_size=self.configs.batch_size, collate_fn=data_collator)
        if sampler:
            sampler = RandomSampler(tokenized_datasets) if not torch.cuda.device_count() > 1 else DistributedSampler(tokenized_datasets)
        else:
            sampler = None
    ...
    def tokenize_function(self, example, tokenizer):
        ...
        """
        # 分词
        token = tokenizer(example["src"], truncation=True, max_length=self.config.sen_max_length, padding='max_length') #configs.padding)
